...
// Update on: 12/12/2014
// Updated by: Shriram Jadhav
...
?>
        ...
        <div class="form-group clearfix test-panal" style="margin-top: 20px;">
             <h4>Individual Performance Chart</h4>    
        ...
        <div class="span2 offset1" >
            ...
            <?php
             ...
             $datesArrFinal = array_keys($data);  
                  ...
                  $empData = array(0 => 'All') + $empData;
             echo CHtml::dropDownList('empNames', $eid, $empData, array('empty' => '(Select a name)'));
          
            ?>
        </div>
                <table style="margin-left: 35%">
    <tr>
        <td><?php echo CHtml::textField('from_date', '', array('id' => 'from_date', 'class' => 'datepicker', 'placeholder' => 'From Date', 'style' => 'width:100px;')); ?></td>
        <td><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;To</label></td>
        <td><?php echo CHtml::textField('to_date', '', array('id' => 'to_date', 'class' => 'datepicker', 'placeholder' => 'To Date', 'style' => 'width:100px;')); ?></td>
        <td align="left"><?php
           ...
           echo CHtml::dropDownList('drp_weekly',$wid,$tdarray,array('empty'=>'Select Value')); 
            ...
            <div style="margin-top:30px; float:left; margin-left:90px;" id="dynGraphs">
               ...
               // Write your logic here
                ...
                $fromD = (isset($_GET['f'])&& $_GET['f']!= '')?$_GET['f']:"";
                $toD = (isset($_GET['t'])&& $_GET['t']!= '')?$_GET['t']:"";
                $catCondArray = array();
                $seriesValuesArray = array();
                if($eid == 0){
                    foreach ($datesArrF as $key => $array){
                        $catCondArray[$key] = $array['empName'];//implode("<br/>",str_split($array['empName']));//
                        $seriesValuesArray[$key] = (int) $array['count'];
                    }
                }
                
                if($daysCount && !empty($eid))
                {    
                ...
                else if(!$monthsCount && !$weeksCount && !$daysCount && $fromD == "" && $toD == "")
                {
                    ?>
                <div style="margin-left:40px;font-size: 16px; color:navy;">NOTE: Please select employee name to view performance chart against daily, weekly or monthly.</div>
                  <?php   
                }
                
                else if($fromD != "" && $toD != "" && $eid == 0)
                {
                     foreach($data as $individualData);
                     {
                         $catCondArray[] = $individualData['empName'];
                         $seriesValuesArray[] = $individualData['count'];
                     }

                }
                else if($fromD != "" && $toD != "" && !empty($eid))
                {
                    $current = new DateTime($fromD);
                    $db_date = new DateTime($toD);
                    $days = $current->diff($db_date)->days;
                    for ($i = 0; $i < ($days); $i++) {
                        $catCondArray[] = date('jS/M', strtotime("-$i days"));
                        $currdate = date('Y-m-d',strtotime("-$i days"));
                        if(in_array($currdate,$datesArrFinal))
                        {
                            $seriesValuesArray[] = $datesArrF[$currdate]; 
                    ...
                    //CHelper::dump($seriesValuesArray);

                }
               
                 if($monthsCount || $weeksCount || $daysCount || $fromD != "" && $toD != "")
                 {
                  ...
                  $this->Widget('application.extensions.highcharts.HighchartsWidget',
                          array(
                                ...
                                'series' => array(
                                                    array('type'=>'bar','name' => 'Performance', 'data' => $seriesValuesArray),
