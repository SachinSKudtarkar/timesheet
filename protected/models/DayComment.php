...
//            $to[] = array("email" => "kpanse@cisco.com", "name" => "Krishnaji");
    ...
    {
        return Yii::app()->db->createCommand("select IF(((select (SUM( `level_hours` ) * 60 * 60) as estimated from tbl_project_level_allocation where project_id = {$project_id})-(select SUM( BIG_TIME_TO_SEC( `hours` )) as utilized from tbl_day_comment where spid = {$project_id})) <= 0, true, false) as result")->queryRow();
