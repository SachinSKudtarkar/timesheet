...
class TbPager extends CLinkPager
{
	...
	public function init()
	{
		...
		if ($this->prevPageLabel === null)
			...
			$this->lastPageLabel = '>>|';
                if (isset(Yii::app()->params['maxButtonCount']))
                    $this->maxButtonCount=Yii::app()->params['maxButtonCount'];
	...
	protected function createPageButtons()
	{
		...
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->firstPageLabel, 0, 'First', $currentPage <= 0, false,true);

		// prev page
		if (($page = $currentPage - 1) < 0)
			$page = 0;

		$buttons[] = $this->createPageButton($this->prevPageLabel, $page, 'Previous', $currentPage <= 0, false, true);
		...
		$buttons[] = $this->createPageButton($this->nextPageLabel, $page, 'Next', $currentPage >= ($pageCount - 1), false, true);

		// last page
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->lastPageLabel, $pageCount - 1, 'Last', $currentPage >= ($pageCount - 1), false, true);
	 ...
	 */
	protected function createPageButton($label, $page, $class, $hidden, $selected, $tooltip = false)
	{
                $tooltipContent = $class;
		if ($hidden || $selected)
			$class .= ' '.($hidden ? 'disabled' : 'active');

		return CHtml::tag('li', array('class'=>$class), CHtml::link($label, $this->createPageUrl($page),($tooltip?array('rel'=>"tooltip","data-title"=>$tooltipContent):'')));
	}
}
