...
class HeaderGroupGridView extends TbGridView {
    ...
    //private $_customHeaderHtmlOptions = array();
        ...
        foreach ($this->columns as $column) {
            for ($m = 0; $m < count($this->mergeHeaders); $m++) {
                if ($i == $this->mergeHeaders[$m]["start"]):
                    ...
                    //$column->headerHtmlOptions[$_customHeaderHtmlOptions] = 'text-align:center';
                    if (isset($this->mergeHeaders[$m]["style"])) {
                        $column->headerHtmlOptions['style'] = $this->mergeHeaders[$m]["style"];
                    }
                    $column->id = NULL;
                    $column->renderHeaderCell();
                endif;
